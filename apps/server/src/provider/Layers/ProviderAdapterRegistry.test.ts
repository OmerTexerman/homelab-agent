...
layer("ProviderAdapterRegistryLive", (it) => {
  it("rejects duplicate provider adapter registrations", () => {
    try {
      indexProviderAdapters([fakeCodexAdapter, { ...fakeCodexAdapter }]);
      assert.fail("expected duplicate provider adapter registration to throw");
    } catch (error) {
      if (!(error instanceof DuplicateProviderAdapterRegistrationError)) {
        throw error;
      }
