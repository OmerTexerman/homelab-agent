...
export function resolveClaudeApiModelId(modelSelection: ModelSelection): string {
  // resolveClaudeContextWindow only honors a stored contextWindow selection
  // when the model's own capabilities declare that option, so models without
  // the descriptor (Fable 5 — 1M is its default — or Haiku) never get the
  // `[1m]` suffix appended even if a stale selection carries one.
  ...
  if (!claudeSettings.enabled) {
    return buildServerProvider({
      ...
      probe: {
        ...
        status: "warning",
        auth: { status: "unknown" },
        message: "Claude is disabled in Providers settings.",
      },
    });
  }
