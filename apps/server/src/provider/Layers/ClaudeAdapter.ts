...
import { makeProviderEventCanonicalizer } from "./ProviderEventCanonicalizer.ts";
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const runSdkStream = (
    ...
    Stream.fromAsyncIterable(context.query, (cause) => {
      const stderrTail = formatStderrTail(context.stderrTail);
      // Surface only the SDK's process-exit text (e.g. "Claude Code process
      // exited with code 1") plus the CLI stderr tail so auth/spend/flag
      // errors reach the user. Anything else stays behind a structural
      // message: arbitrary stream failures can carry credential material
      // that must remain in the cause chain, never in user-visible text.
      const message =
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      const claudeBinaryPath = runtimeEnvironment?.commandPath ?? claudeSdkExecutablePath;
