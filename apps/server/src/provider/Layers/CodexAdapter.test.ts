...
function makeThreadRuntimeLaunchContext(input: {
  ...
  return {
    ...
    hostBinDir: NodePath.join(input.baseDir, "bin"),
    shellWrapperPath: NodePath.join(input.baseDir, "bin", "runtime-shell"),
  };
}
...
validationLayer("CodexAdapterLive validation", (it) => {
  ...
  it.effect("starts Codex through the project runtime wrapper when launch context exists", () => {
    ...
    return Effect.gen(function* () {
      ...
      NodeAssert.equal(session.cwd, "/workspace");
      NodeAssert.equal(runtimeInput.binaryPath, NodePath.join(launchContext.hostBinDir, "codex"));
      NodeAssert.equal(runtimeInput.cwd, "/workspace");
      NodeAssert.equal(runtimeInput.processCwd, launchContext.hostWorkspacePath);
      NodeAssert.equal("homePath" in runtimeInput, false);
    }).pipe(
...
lifecycleLayer("CodexAdapterLive lifecycle", (it) => {
  ...
  it.effect("normalizes open tool lifecycle when a turn completes before item completion", () =>
    Effect.gen(function* () {
      ...
      NodeAssert.equal(
        events[1]?.eventId,
        "evt-turn-completed-before-command:item-completed:cmd_1",
      );
      NodeAssert.equal(events[1]?.itemId, "cmd_1");
    }),
  );
