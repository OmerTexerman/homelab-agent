...
function hangingAuthCommandSpawnerLayer(input: {
  readonly authArgs: string;
  readonly versionStdout: string;
  readonly killCalls: Ref.Ref<number>;
}) {
  return Layer.succeed(
    ChildProcessSpawner.ChildProcessSpawner,
    ChildProcessSpawner.make((command) => {
      ...
      if (joined === "--version") {
        return Effect.succeed(mockHandle({ stdout: input.versionStdout, stderr: "", code: 0 }));
      }
      if (joined !== input.authArgs) {
      ...
      return Effect.gen(function* () {
        const handle = ChildProcessSpawner.makeHandle({
          ...
          kill: () => Ref.update(input.killCalls, (current) => current + 1),
          unref: Effect.succeed(Effect.void),
          stdin: Sink.drain,
          stdout: Stream.never,
          stderr: Stream.never,
...
it.layer(Layer.mergeAll(NodeServices.layer, ServerSettingsService.layerTest()))(
  ...
  (it) => {
    ...
    describe("checkCodexProviderStatus", () => {
      ...
      it.effect("allows codex auth probes to use the shared auth timeout", () =>
        Effect.gen(function* () {
          yield* withTempCodexHome();
          const killCalls = yield* Ref.make(0);
          const statusFiber = yield* checkCodexProviderStatus().pipe(
            Effect.provide(
              hangingAuthCommandSpawnerLayer({
                authArgs: "login status",
                versionStdout: "codex 1.0.0\n",
                killCalls,
              }),
            ),
            Effect.forkChild,
    ...
    describe("checkClaudeProviderStatus", () => {
      ...
      it.effect("allows claude auth probes to use the shared auth timeout", () =>
        Effect.gen(function* () {
          const killCalls = yield* Ref.make(0);
          const statusFiber = yield* checkClaudeProviderStatus().pipe(
            Effect.provide(
              hangingAuthCommandSpawnerLayer({
