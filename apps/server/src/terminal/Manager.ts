...
import { PtyAdapter, PtySpawnError, type PtyExitEvent, type PtyProcess } from "./PtyAdapter.ts";
...
// mount instead of the user's real environment. The scrub is gated on an
  ...
  function* (options: TerminalManagerOptions) {
    ...
    const historyPath = (threadId: string, terminalId: string) => {
      ...
      if (terminalId === DEFAULT_TERMINAL_ID) {
        return NodePath.join(logsDir, `${threadPart}.log`);
      }
      return NodePath.join(logsDir, `${threadPart}_${toSafeTerminalId(terminalId)}.log`);
    };

    const legacyHistoryPath = (threadId: string) =>
      NodePath.join(logsDir, `${legacySafeThreadId(threadId)}.log`);
    ...
    const deleteAllHistoryForThread = Effect.fn("terminal.deleteAllHistoryForThread")(function* (
      ...
      yield* Effect.forEach(
        ...
        (name) =>
          fileSystem.remove(NodePath.join(logsDir, name), { force: true }).pipe(
            Effect.catch((error) =>
...
export const layer = TerminalManagerLive;
