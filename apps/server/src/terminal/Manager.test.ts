...
function historyLogPath(logsDir: string, threadId = "thread-1"): string {
  return NodePath.join(logsDir, historyLogName(threadId));
}
...
function multiTerminalHistoryLogPath(
  ...
  return NodePath.join(logsDir, multiTerminalHistoryLogName(threadId, terminalId));
}
...
it.layer(NodeServices.layer, { excludeTestServices: true })("TerminalManager", (it) => {
  ...
  it.effect("preserves non-notFound cwd stat failures", () =>
    Effect.gen(function* () {
      ...
      const blockedRoot = NodePath.join(baseDir, "blocked-root");
      const blockedCwd = NodePath.join(blockedRoot, "cwd");
  ...
  it.effect("propagates explicit worktree metadata through snapshots and lifecycle events", () =>
    Effect.gen(function* () {
      ...
      const firstWorktreePath = NodePath.join(baseDir, "worktrees", "feature-a");
      const secondWorktreePath = NodePath.join(baseDir, "worktrees", "feature-b");
  ...
  it.effect("preserves worktree metadata when reopening an exited session", () =>
    Effect.gen(function* () {
      ...
      const worktreePath = NodePath.join(baseDir, "worktrees", "feature-a");
  ...
  it.effect("migrates legacy transcript filenames to terminal-scoped history path on open", () =>
    Effect.gen(function* () {
      ...
      const legacyPath = NodePath.join(logsDir, "thread-1.log");
