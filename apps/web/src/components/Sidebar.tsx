...
import type { SidebarDraftAwareThreadSummary as SidebarThreadSummary } from "./Sidebar.logic";
import {
...
export const SidebarThreadRow = memo(function SidebarThreadRow(props: SidebarThreadRowProps) {
  ...
  const gitStatus = useEnvironmentQuery(
    showSourceControlUi && thread.branch != null && gitCwd !== null
              ...
              onClick: () => {
                void (async () => {
                  ...
                  const compatibilityWorkspaceLine = showCompatibilityWorkspaceControls
                    ? [`Compatibility workspace: ${member.workspaceRoot}`]
                    : [];
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>
              {projectRenameTarget
                ? `Update the title for project "${projectRenameTarget.title}".`
...
const SidebarProjectsContent = memo(function SidebarProjectsContent(
  ...
  const navigate = useNavigate();
  const { isMobile, setOpenMobile } = useSidebar();
  const createScratchThreadCommand = useAtomCommand(standaloneThreadEnvironment.create, {
    reportFailure: false,
  });
  ...
  const createScratchThread = useCallback(async () => {
    ...
    const threadId = newThreadId();
    const result = await createScratchThreadCommand({
...
export default function Sidebar() {
  const allProjectsIncludingCurator = useProjects();
  const allSidebarThreadsIncludingCurator = useThreadShells();
  ...
  const handleProjectDragEnd = useCallback(
    ...
    [normalSidebarProjects, orderedProjects, reorderProjects, sidebarProjectSortOrder],
  );
  ...
  const prewarmedSidebarThreadKeys = useMemo(
    () =>
      getSidebarThreadIdsToPrewarm(
        visibleSidebarThreadKeys.filter((threadKey) => {
          const entry = sidebarThreadByKey.get(threadKey);
          return !(entry && "isDraft" in entry && entry.isDraft === true);
        }),
      ),
