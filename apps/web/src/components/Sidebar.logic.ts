...
export function resolveThreadStatusPill(input: {
  ...
  // decisionQueue already derives "plan-follow-up" from the same signals
  // upstream added here (interactionMode, hasActionableProposedPlan,
  // isLatestTurnSettled) and via its priority ordering never wins over an
  // active provider-user-input decision, so this stays on the shared
  // decision-queue read model instead of duplicating the check inline.
