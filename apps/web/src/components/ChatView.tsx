...
function ChatViewContent(props: ChatViewProps) {
  ...
  const serverSessionLastError = activeServerThread?.session?.lastError ?? null;
      ...
      if (reconnectingThroughVersionSkew) {
        ...
        items.push({
          ...
          description:
            ...
            HOMELAB_PRODUCT_COPY.serverConnection.chatActionsDescription,
          actions: (
            <>
              <Button
                size="xs"
  ...
  const gitStatusQuery = useEnvironmentQuery(
    !showSourceControlUi || gitStatusCwd === null
      ? null
  ...
  const onOpenTurnDiff = useCallback(
    (turnId: TurnId, filePath?: string) => {
      if (!isServerThread || !showSourceControlUi || !activeThreadRef) return;
      useDiffPanelStore.getState().selectTurn(activeThreadRef, turnId, filePath);
      useRightPanelStore.getState().open(activeThreadRef, "diff");
      onDiffPanelOpen?.();
    },
    [activeThreadRef, isServerThread, onDiffPanelOpen, showSourceControlUi],
  );
    ...
    <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden bg-background">
      ...
      {!shouldUseRightPanelSheet && rightPanelOpen && activeThreadRef ? (
        <RightPanelTabs
          ...
          // Always openable: the panel renders a "waiting for the thread to start" state.
          memoryAvailable
