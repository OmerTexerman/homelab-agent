...
export const HOMELAB_PRODUCT_COPY = {
  ...
  curator: {
    ...
    kickoffPrompt: [
      ...
      "",
      "Take inventory first (`homelab curate overview`, `homelab snapshot`, `homelab curate memory --all`, `homelab curate skills`), verify whatever is cheap to verify against the live homelab, and report back with a short prioritized triage list:",
      "",
      "- duplicate or contradictory entities, relations, and memory entries",
      "- stale or unverifiable entries (note what you probed and what answered)",
      "- skills that no longer match the infrastructure they describe",
      "- memory superseded in practice but never corrected in the record",
      "",
      "Propose the specific corrections you would make (with reasons), but wait for my go-ahead before changing anything.",
    ].join("\n"),
  },
  projectRuntime: {
