...
| `apps/server/src/runtime/Layers/RuntimeExecutionContext.ts`                                                                     | Runtime ids, storage layout, Docker/container naming, generated runtime env, wrapper paths, mount specs, auth sync descriptors, and conversion to execution context.                                   | Docker lifecycle operations, bootstrap registry persistence, provider policy, terminal session state, or homelab knowledge materialization. |
...
- Snapshot restore: checkpoint projection can identify restore targets and stale
  checkpoint refs, but filesystem/container restore behavior still needs a full
  hardened path.
